...
import org.profamilia.hc.view.backing.clinico.consultorio.RegistrarEvolucionConsultaBean;
import org.profamilia.hc.view.constantes.IConstantes;
import org.profamilia.hc.view.constantes.IMsg;
import org.profamilia.hc.view.util.FacesUtils;


public class ConsultarFoliosSeguimientoBean extends BaseBean {

    private Chusuario usuario;

    /** Almacena el bean de Registrar Folios */

    RegistrarEvolucionConsultaBean registrarFolio;
    ...
    public ConsultarFoliosSeguimientoBean() {
    }
    ...
    public void consultarFoliosUsuario() {
        if (usuario.getHuslnumero() != null) {
            try {
                lstFolios = 
                        this.getServiceLocator().getClinicoService().getFoliosUsuarioEvolucion(usuario.getHuslnumero());
            } catch (ModelException e) {
                e.printStackTrace();
            }
    ...
    public String RegistrarEvolucion() {
        ...
        registrarFolio = 
                (RegistrarEvolucionConsultaBean)FacesUtils.getManagedBean("registrarEvolucionConsultaBean");
        if (consulta != null && registrarFolio != null) {
            registrarFolio.setConsulta(consulta);

        }
        return BeanNavegacion.RUTA_REGISTRAR_SEGUIMIENTO_CONSULTA_EXTERNA;
    }
