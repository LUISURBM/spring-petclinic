...
public class ConsultarUsuarioEvolucionBean extends BaseBean {
    ...
    ConsultarFoliosSeguimientoBean cosultaFolios;

    private boolean existenCriterios;


    public ConsultarUsuarioEvolucionBean() {
    }
    ...
    public String consultarEvolucion() {
        // Obtenemos el usuario seleccionado 
        usuarioClon = (Chusuario)this.dtConsultaUsuario.getRowData();
        dtConsultaUsuario.setFirst(IConstantes.OFFSET_DATATABLE);
        if (usuarioClon != null) {
            cosultaFolios = 
                    (ConsultarFoliosSeguimientoBean)FacesUtils.getManagedBean("consultarFoliosSeguimientoBean");

            if (cosultaFolios != null) {
                cosultaFolios.setUsuario(usuarioClon);
                return BeanNavegacion.RUTA_CONSULTAR_FOLIOS_SEGUIMIENTO;
            }
        }
        return null;
    }
