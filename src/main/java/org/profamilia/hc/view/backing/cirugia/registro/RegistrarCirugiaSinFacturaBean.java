...
public class RegistrarCirugiaSinFacturaBean extends BaseBean {
    ...
    public RegistrarCirugiaSinFacturaBean() {
    }
        ...
        if (usuario != null) {
            ...
            try {
                lstCirugiasRegistradas = 
                        this.serviceLocator.getCirugiaService().getCirugiasSinFactura(usuario);
            ...
            if (lstCirugiasRegistradas == null || 
                ...
                lstCirugiasRegistradas = new ArrayList<Chregicirpro>();
            }
     ...
     */
    public void setLstCirugiasRegistradas(List<Chregicirpro> lstCirugiasRegistradas) {
     ...
     * @return
     */
    public List<Chregicirpro> getLstCirugiasRegistradas() {
    ...
    public String agregarCirugia() {
        ...
        System.out.println("Entro1" + wejecuto + navegationRule);
        if (!wejecuto) {
        System.out.println("Entro2" + wejecuto + navegationRule);
         
            navegationRule = "";
            wejecuto = true;
            System.out.println("Entro3" + wejecuto + navegationRule);
            ...
            if (regcirpro == null || regcirpro.getHrcndonant() == null) {
                FacesUtils.addErrorMessage("body:formRegistrarCirugia:panelTabRegistrarCirugia:mnuDonantes", 
                                           MSG_CAMPO_OBLIGATORIO);
                wexito = false;
            }

            if (regcirpro == null || regcirpro.getHrcdfeccirpro() == null) {
                FacesUtils.addErrorMessage("body:formRegistrarCirugia:panelTabRegistrarCirugia:fechaCirugia", 
                                           MSG_CAMPO_OBLIGATORIO);
                wexito = false;
            }

            if (regcirpro == null || regcirpro.getHrcnprogra() == null) {
                FacesUtils.addErrorMessage("body:formRegistrarCirugia:panelTabRegistrarCirugia:mnuProgramas", 
                                           MSG_CAMPO_OBLIGATORIO);
                wexito = false;
            }

            if (regcirpro == null || regcirpro.getHrcntipusu() == null) {
                FacesUtils.addErrorMessage("body:formRegistrarCirugia:panelTabRegistrarCirugia:mnuTipoUsu", 
                                           MSG_CAMPO_OBLIGATORIO);
                wexito = false;
            }

            if (lstCirugias == null || lstCirugias.isEmpty()) {
                FacesUtils.addErrorMessage("body:formRegistrarCirugia:panelTabRegistrarCirugia:imagelupa", 
                                           MSG_CAMPO_OBLIGATORIO);
                wexito = false;
            }
            ...
            if (regcirpro.getHrccservic() == null || 
                regcirpro.getHrccservic().equals("")) {
                FacesUtils.addErrorMessage("Error, Por favor verificar el servicio", 
                                           MSG_CAMPO_OBLIGATORIO);
                wexito = false;
            } else {
                try {
            ...
            try {
                tipodocu = 
                        this.serviceLocator.getCirugiaService().getTipoDocuPorId(IConstantes.CDOC_CISFAC);
            } catch (ModelException e) {
                e.printStackTrace();
            }

            if (!lstCirugias.isEmpty() && wexito) {

                Iterator it = lstCirugias.iterator();
                int i = 0;
                while (it.hasNext()) {
                    it.next();
                    servicioAux = lstCirugias.get(i);
                    try {
                        this.serviceLocator.getCirugiaService().saveCirugiaSinFactura(regcirpro, 
                                                                                      usuario, 
                                                                                      servicioAux, 
                                                                                      tipodocu, 
                                                                                      userName());
            ...
            if (wexito) {
                ...
                ProgramarCirugiaBean programarCirugiaUsuario;

                programarCirugiaUsuario = 
                        (ProgramarCirugiaBean)FacesUtils.getManagedBean("programarCirugiaBean");

                if (programarCirugiaUsuario != null) {
                    programarCirugiaUsuario.setUsuario(usuario);

                    programarCirugiaUsuario.init();
                }
                ...
                navegationRule = BeanNavegacion.RUTA_IR_PROGRAMACION_CIRUGIA;
                registrarCirugia = true;
                FacesUtils.resetManagedBean("registrarCirugiaSinFacturaBean");
            } else {
                navegationRule = "";
    ...
    public String seleccionarCirugia() {
        regcirpro = new Chregicirpro();
        regcirpro = (Chregicirpro)dtCirugiasRegistradas.getRowData();
        Cpservicio servicioAux = new Cpservicio();
        try {
            servicioAux = 
                    this.serviceLocator.getClinicoService().getServicioPorId(regcirpro.getHrccservic().getCsvccodigo());
    ...
    public String confirmarCirugia() {
        ...
        if (wexito) {
            try {
                this.serviceLocator.getCirugiaService().confirmarCirugiaSinFactura(regcirpro);
                FacesUtils.addInfoMessage(MSG_ADICION);
                FacesUtils.resetManagedBean("registrarCirugiaSinFacturaBean");
                FacesUtils.resetManagedBean("consultarUsuarioCirugiaSinFacturaBean");
                navegationRule = 
    ...
    public void setRegcirpro(Chregicirpro regcirpro) {
        this.regcirpro = regcirpro;
    }

    public Chregicirpro getRegcirpro() {
        return regcirpro;
    }
