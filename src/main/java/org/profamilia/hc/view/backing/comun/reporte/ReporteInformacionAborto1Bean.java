...
public class ReporteInformacionAborto1Bean extends BaseBean implements JRDataSource {
    ...
    BigDecimal ANNO_PROCEDIMIENTO;
    BigDecimal COD_CLININICA;
    String NOMBRE_CLINICA;
    String ESTADO_CIVIL;

    public ReporteInformacionAborto1Bean() {
    }

    public void init() {
        inicializarReporteInformacionAborto1();
    }

    public void inicializarReporteInformacionAborto1() {
    ...
    public Object getFieldValue(JRField jrField) {
        ...
        else if (fieldName.equals("SEMA_EMB_CONFIR"))
            value = SEMA_EMB_CONFIR;
        ...
        else if (fieldName.equals("ANNO_PROCEDIMIENTO"))
            value = ANNO_PROCEDIMIENTO;
    ...
    public void obtenerDatosReporte(int indexAux) {
        ...
        if (aux != null) {
            ...
            if (aux[5] != null) {
                SEMA_EMB_CONFIR = new BigDecimal(aux[5].toString());
            }

            if (aux[6] != null) {
                NUMERO_GESTACIONES = new BigDecimal(aux[6].toString());
            }

            if (aux[7] != null) {
                NUMERO_CESARIAS = new BigDecimal(aux[7].toString());
            }

            if (aux[8] != null) {
                NUMERO_ABORTOS = new BigDecimal(aux[8].toString());
            }

            if (aux[9] != null) {
                NUMERO_PARTOS = new BigDecimal(aux[9].toString());
            }

            if (aux[10] != null) {
                METODO_PLANIFICA_ANTERIOR = aux[10].toString();
            }

            if (aux[11] != null) {
                METODO_PLANIFICA_POSTERIOR = aux[11].toString();
            }

            if (aux[12] != null) {
                FECHA_PROCEDIMIENTO = (Date)aux[12];
            }
            
            if (aux[13] != null) {
                ANNO_PROCEDIMIENTO = new BigDecimal(aux[13].toString());
            }
            
            if (aux[14] != null) {
                COD_CLININICA = new BigDecimal(aux[14].toString());
            }
            
            if (aux[15] != null) {
                NOMBRE_CLINICA = aux[15].toString();
            }
            
            if (aux[16] != null) {
                ESTADO_CIVIL = aux[16].toString();
            }
        }
    }
    ...
    public void generarReporte() {
        ...
        String urlAux = "";
        generoReporte = false;
        String nombSalida = "ReporteInformacionAborto1_" + new Date();


        urlAux = "/reports/reporteInformacionAborto1.jasper";


        try {
        lstReporte = this.serviceLocator.getClinicoService().getReporteInformacionAborto1(fechaInicial,fechaFinal,wcodclin);
         } catch (ModelException e) {
           e.printStackTrace();
         }
