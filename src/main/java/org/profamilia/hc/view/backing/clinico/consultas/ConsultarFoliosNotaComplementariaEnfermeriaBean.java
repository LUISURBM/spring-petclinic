...
import org.profamilia.hc.view.backing.clinico.consultorio.RegistrarNotaComplementariaEnfermeriaBean;
...
public class ConsultarFoliosNotaComplementariaEnfermeriaBean extends BaseBean {

    private Chusuario usuario;

    /** Almacena el bean de Registrar Folios */

    RegistrarNotaComplementariaEnfermeriaBean registrarFolio;
    ...
    public ConsultarFoliosNotaComplementariaEnfermeriaBean() {
    }
    ...
    public void consultarFoliosUsuario() {
        if (usuario.getHuslnumero() != null) {
            try {
                lstFolios = 
                        this.getServiceLocator().getClinicoService().getFoliosUsuarioNotaComplementariaEnfemeria(usuario.getHuslnumero(),userName());
            } catch (ModelException e) {
                e.printStackTrace();
            }
    ...
    public String RegistrarNotaMedica() {
        ...
        registrarFolio = 
                (RegistrarNotaComplementariaEnfermeriaBean)FacesUtils.getManagedBean("registrarNotaComplementariaEnfermeriaBean");
        if (consulta != null && registrarFolio != null) {
            registrarFolio.init();
            registrarFolio.setConsulta(consulta);

        }
        return BeanNavegacion.RUTA_REGISTRAR_NOTA_COMPLEMENTARIA_ENFERMERIA;
    }
