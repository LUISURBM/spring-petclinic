...
public class SolicitarNotaAclaratoriaBean extends BaseBean {
    //-----------------------------------------------------------------------
    ...
    public SolicitarNotaAclaratoriaBean() {
    }
    ...
    public void consultarNotaAclaratoria() {
        try {
            if (numeroUsuario != null) {
                lstDescripcion = 
                        this.serviceLocator.getCirugiaService().getSolicitudFoliosNotaAclaratoria(numeroUsuario, userName());
            }
    ...
    public String solicitarAclaracion() {
        try {
            ...
            this.serviceLocator.getCirugiaService().saveAutorizacionNotaAclaratoria(autoriza);
            email.sendSolicitudMailNotaAclaratoria(autoriza,recipientTo,recipientCc);
    ...
    /**
       * @return
       */
      public String aclaracionNotaAclaratoria() {
          ...
          if (aux.getHcolnumero() != null) {
              autoriza.setHancoperasolic(userName());
              autoriza.setHandfechasolic(new Date());
              autoriza.setHannclinica(getClinica().getCclncodigo());
              autoriza.setHannnumerconsu(aux.getHcolnumero());
              autoriza.setHandfechainici(new Date());
              autoriza.setHandfechafinal(fechafina.getTime());
              if(aux.getHcocservicio()!= null){
                  autoriza.setHancservicio(aux.getHcocservicio().getCsvccodigo());    
                  autoriza.setHancnombserv(aux.getHcocservicio().getCsvcnombre());
              }
              
              if(aux.getHcolusuario() != null ){
                  autoriza.setHancnombpaci(aux.getHcolusuario().getHuscprimernomb() +  aux.getHcolusuario().getHuscprimerapel());
                  autoriza.setHannnumeiden(aux.getHcolusuario().getHusanumeiden());           
                  autoriza.setHanctipoiden(aux.getHcolusuario().getHusetipoiden());
                  
              }
              
              autoriza.setHandfechaconsu(aux.getHcodfecregistr());
          ...
          try {
              autorAux = 
                      this.serviceLocator.getCirugiaService().getAutorizacionNotaAclaratoria(autoriza);
          } catch (ModelException e) {
              e.printStackTrace();
          }

          if (autorAux == null) {
              return BeanNavegacion.RUTA_SOLICITAR_NOTA_ACLARATORIA;
          } else {
              FacesUtils.addErrorMessage(MSG_SOLICITUD_NOTA_ACLARACION_EN_PROCESO);
              return BeanNavegacion.RUTA_ACTUAL;
          }
    ...
    public void setAutoriza(Chautornota autoriza) {
        this.autoriza = autoriza;
    }

    public Chautornota getAutoriza() {
        return autoriza;
    }
}
