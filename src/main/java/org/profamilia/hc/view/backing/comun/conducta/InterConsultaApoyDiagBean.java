...
import org.profamilia.hc.view.backing.comun.buscador.BuscadorCupsBean;
...
public class InterConsultaApoyDiagBean extends BaseBean implements JRDataSource {

    //-----------------------------------------------------------------------
    //  ATRIBUTOS DE INSTANCIA
    //-----------------------------------------------------------------------

    static final int ACTIVAR_INTER_CONSULTA = 0;
    static final int ACTIVAR_BUSCADOR_CUPS = 1;
    private String descripcion;
    private boolean mostrarCups;
    private List<Crcup>lstCups;
    ...
    public InterConsultaApoyDiagBean() {
    }
    ...
    public String buscarCup(){
        crcup = null;
        descripcionCups = null;
                try {
                    crcup = this.serviceLocator.getHistoriaService().getCupByCodigo(codigoCups);
                    if(crcup!=null){
                        descripcionCups =crcup.getCrcuvdescrip();
                    }
                    else{
                        FacesUtils.addErrorMessage("el Servicio con c�digo " +codigoCups+" no existe"); 
                    }
    ...
    public void agregarCup() {    
        if(validarCampos()){
            if(lstCups==null){
                    lstCups = new ArrayList<Crcup>();
            }
            lstCups.add(crcup);
            codigoCups = null;
            descripcionCups = null;
    ...
    public boolean validarCampos(){
        if(codigoCups == null || descripcionCups == null || crcup == null){
            FacesUtils.addErrorMessage("Falta informaci�n obligatoria"); 
            return false;
        }
        return true;
    }
...
*/
    ...
    public void consultarCups() {
        FacesUtils.resetManagedBean("buscadorCupsBean");
        BuscadorCupsBean buscador = 
            (BuscadorCupsBean)FacesUtils.getManagedBean("buscadorCupsBean");
        if (buscador != null) {
            buscador.init();
        }
        selectedIndex = ACTIVAR_BUSCADOR_CUPS;
        mostrarCups = true;

    }
    
    public void aceptarCups() {
        BuscadorCupsBean buscador = 
            (BuscadorCupsBean)FacesUtils.getManagedBean("buscadorCupsBean");
        if (buscador != null) {
            if (lstCups != null && 
                !lstCups.isEmpty()) {
               lstCups.addAll(buscador.getLstCrcupsSelec());
            }
            else{
                lstCups = buscador.getLstCrcupsSelec();
            }
        }
        selectedIndex = ACTIVAR_INTER_CONSULTA;
        mostrarCups = false;
        FacesUtils.resetManagedBean("buscadorCupsBean");
    }
    
    public void eliminarCups() {
        Crcup aux = (Crcup)dtCups.getRowData();
        lstCups.remove(aux);
    ...
    public void setMostrarCups(boolean mostrarCups) {
        this.mostrarCups = mostrarCups;
    }

    public boolean isMostrarCups() {
        return mostrarCups;
    }

    public void setLstCups(List<Crcup> lstCups) {
        this.lstCups = lstCups;
    }

    public List<Crcup> getLstCups() {
        return lstCups;
    }
